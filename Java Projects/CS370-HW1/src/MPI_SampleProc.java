...
public class MPI_SampleProc extends MPI_Proc {
    ...
    public void exec(int argc, String argv[]) throws InterruptedException
    {
        ...
        if (rank == 0) { // Generator
        	...
        	//send number of tasks to root.
        	buffer[1] = Integer.toString(availableTasks.length);
        	MPI_Send(buffer, 2, MPI_STRING, 8, TASK_TAG, MPI_COMM_WORLD);  
        	...
        	//get number of tasks
        	MPI_Recv(buffer, 2, MPI_STRING, 0, TASK_TAG, MPI_COMM_WORLD, status);
        	int numTasks = Integer.parseInt(buffer[1]);
        	
        	//for all tasks, receive that number of results from anywhere
